      ...
      <div id="popup" style="display:none"> <!-- 초기화면에서는 보여줄 필요가 없으니 아예 생성도 하지 않도록 한다. -->
        <div class="popup-body">
          ...
          <div class="countdown-message">
            <span id="ncounter"></span> 후 자동 퇴실합니다.
          </div>
          <div class="d-flex justify-content-end exit-modal-footer">
            <input class="btn btn-success" type="button" value="더 할래요~" @click="counter_reset(), close_window()">
            <button type="button" class="btn btn-danger" data-bs-dismiss="modal" @click="exit()">그만 할래요</button>
          </div>
        </div>
      </div>
      <!-- Layer Popup 설정 끝 -->
...
export default {
  ...
  methods: {
    ...
    counter_init: function() { // 메인화면 세션 카운트 실행
    ...
    nCounter_init: function() { // 팝업화면 추가 세션 카운트 실행
      this.isIntendedExit = false;
      this.tid = setInterval(this.nCounter_run, 1000);
    },

    nCounter_run: function() { // 팝업화면 추가 세션 카운트
    ...
    counter_reset: function() { // 메인화면 카운트 재시작 및 서버 세션 연장
      //(1) 세션 카운트 초기화
