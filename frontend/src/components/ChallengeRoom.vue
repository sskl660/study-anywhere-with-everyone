    ...
    <div class="d-flex row wider justify-content-center" id="body">
      <div class="joinbox">
        ...
        <li class="changebtn">
          ...
          <!-- 가입하기 버튼 -->
          <div
            v-if="!overStartDate && !didJoin() && !overMember()"
            class="Cjoin_btn"
            @click="hidebtn(chall_info.challengeNo, userEmail)"
          >
            ...
            <table class="table">
              ...
              <tbody>
                <tr v-for="(person, index) in chall_info.challengeGroup" :key="person">
                  <th scope="row" style="background-color: #b7beda">
                    <div class="namepoint d-flex align-items-center justify-content-center">
                      <div>{{ person[1] }}</div>
                    </div>
                  </th>
                  <td v-for="taskIdx in chall_info.challengeTaskCnt" :key="taskIdx">
                    ...
                    <router-link
                      :to="{
                        path: '/postDetail',
                        query: { idx: taskIdx - 1, cn: chall_info.challengeNo },
                      }"
                      v-if="
                        task_info[index].taskNo[taskIdx - 1] == -1 &&
                          task_info[index].userEmail == userEmail
                      "
                    >
